...
describe "ActiveRecord American Gladiator" do
  ...
  context "Hang Tough" do
    it "returns orders for 3 users in 2 queries (aka: Remove the N+1 query)" do
      ...
      # Changeable Start
      users = User.joins(:orders).select("orders.amount as amount").where("amount != 3")
      # Changeable End
  ...
  context "Breakthrough and Conquer" do
    it "returns all orders with Footballs and Wrestling Rings" do
      ...
      # end
      orders = Order.joins(:items).where("items.name != ?", "Sweat")
      # Changeable End
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      #   Item.find(id)
      # end
      most_popular_items = Item.joins(:orders).select("items.*, count(items.id) as item_count").group("items.id").order("item_count desc").limit(2)
      # Changeable Stop
